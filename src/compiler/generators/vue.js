...
function generateVue(astRoot, currentFilePath) {
    let cssMap = {};
    let typeCounters = {};
    ...
    // Auto-resolve Server Imports
    if (astRoot.script.imports) {
        astRoot.script.imports.forEach(imp => {
            if (imp.source.endsWith('.ygg')) {
                // It's a Yggdra file, let's peek inside
                try {
                    // Resolve path relative to current file
                    const targetPath = path.resolve(path.dirname(currentFilePath), imp.source);
                    if (fs.existsSync(targetPath)) {
                        const src = fs.readFileSync(targetPath, 'utf8');
                        const parsed = parse(src);

                        if (parsed.server) {
                            // Transform Server definition into Client Service definition
                            const srv = {
                                kind: 'Service',
                                name: imp.alias, // Use the alias as the service name
                                baseUrl: `http://localhost:${parsed.server.portOrBaseUrl}`, // Assumption: localhost for dev
                                body: []
                            };

                            // Synthesize Routes
                            parsed.server.routes.forEach(r => {
                                // Reconstruct the line "get ping /ping"
                                // FIX: Don't add '?' if already present in arg
                                let line = `${r.method} ${r.name} ${r.path}`;
                                r.args.forEach(arg => line += ` ${arg}`);
                                srv.body.push(line);
                            });

                            astRoot.script.services = astRoot.script.services || [];
                            astRoot.script.services.push(srv);
                            imp._handled = true;
                        }
                    }
                } catch (e) {
                    console.warn(`Could not resolve import ${imp.source}:`, e.message);
                }
            }
        });
    }
    ...
    // --- IMPORTS (COMPOSANTS & SERVICES) ---
    const imports = (astRoot.script.imports || []).filter(i => !i._handled);
    const serviceNames = (astRoot.script.services || []).map(s => s.name);

    // 1. Gestion des imports de composants
    const componentImports = imports.filter(i => !serviceNames.includes(i.source));
    if (componentImports.length > 0) {
        scriptContent += componentImports.map(i => {
            // Si c'est un chemin relatif (commence par .), on ajoute .vue
            // Sinon on suppose que c'est une lib externe (peu probable pour l'instant) ou un alias
            let source = i.source;
            if (source.startsWith('.')) {
                source += '.vue';
            }
            return `import ${i.alias} from '${source}';`;
        }).join('\n') + "\n\n";
    }
    ...
    if (astRoot.script.functions.length > 0) {
        scriptContent += astRoot.script.functions.map(f => {
            ...
            f.body.forEach(line => {
                ...
                // Python-style negative indexing support: arr[-1] -> arr.at(-1)
                content = content.replace(/\[-(\d+)\]/g, '.at(-$1)');
                ...
                if (content.startsWith('wait ')) {
                    const delay = content.split(' ')[1];
                    content = `await new Promise(r => setTimeout(r, ${delay}));`;
                    isAsync = true;
                }
